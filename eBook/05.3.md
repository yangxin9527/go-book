...
每一个 `case` 分支都是唯一的，从上至下逐一测试，，直到算法匹配到某个 `case` 或者进入 `default` 条件为止。）

一旦成功地匹配到某个分支，在执行完相应代码后就会退出整个 `switch` 代码块，不需要使用 `break` 语句来结束。

因此，程序也不会自动地去执行下一个分支的代码。如果希望继续执行后续分支的代码，可以使用 `fallthrough` 来达到目的。
...
```
可选的 `default` 分支可以出现在任何顺序，但最好将它放在最后。它
